...
Content that is displayed in the Feed comes from three primary sources: organizational news published in SharePoint, posts in Viva Engage communities, and videos in Stream that are shared with the entire organization or targeted to user groups. Individual content items in the Feed display for a 30-day period.

1. **News published on organizational news sites in SharePoint**

   SharePoint news published from [organizational news sites](/SharePoint/organization-news-site) display in the Feed. Organizational news sites are communication sites designated as a source of authoritative news in the SharePoint admin center. In addition to organizational news posts, the following news will also display in the Feed:
...
3. **Videos in SharePoint hosted by Stream**
   ...
   [Stream videos](/stream/streamnew/new-stream) built on SharePoint or OneDrive that are shared with your entire organization appear in the Feed. Depending on how your organization stores and shares videos in Stream will affect the viewing experience for videos in the Feed.
...
For mobile, the Feed refreshes each time the application is selected. Users can also manually refresh the Feed through the pull-down action. On the web, the Feed automatically refreshes each time the web page is refreshed. Individual content items in the Feed display for a 30-day period. Boosted SharePoint news posts display prominently for up to four days. [Learn more about the news boost feature](https://support.microsoft.com/office/boost-sharepoint-news-from-organization-news-sites-46ad8dc5-8f3b-4d81-853d-8bbbdd0f9c83).

## When can we expect to see a newly created post in the Feed?

Posts will typically take up to 1 hour to appear in a user’s Feed. Content from a newly created [SharePoint home site](home-site-plan.md) is sometimes delayed for up to 24 hours after the site is initially created. After the initial seven day period is over, it will take up to one hour for the post to appear.

## How is the Feed personalized and how is content ranked?

There are several factors such as the content’s age, whether it’s been "boosted" by the organization, the publishing source, and the author’s relationship to the reader to determine the content ranking. This is so users can discover new content each time they open the Feed so it never gets boring or predictable. However, engaging content in the Feed relies on how often content is created on the SharePoint sites and Viva Engage communities.
...
2. Viva Engage Posts that aren't Featured or to **All Company** communities or **Announcements** might be removed and replaced in subsequent feed views to give users more dynamic content.
...
There’s no configuration required to get the Feed working. For the current release, customers can affect content placement in the Feed by targeting content to audiences, or by promoting it.
...
Users on the Feed can do the following actions on posts:
